...
const TaskCard = ({ task, index }) => {
  ...
  const handleDeleteCard = async()=>{
    try{
      dispatch(deleteATaskRequest());
  console.log()
      const res = await axios.delete(`/api/v1/tasks/delete-task/${task._id}`);
      // console.log(res.data)
      dispatch(deleteATaskSuccess(res.data));
      toast.success('Task Deleted Successfully')
    }catch(error){
          // console.log("err", error);
          let htmlError = extractErrorMessage(error.response?.data);
          dispatch(  deleteATaskFailure,
  ...
  return (
    <>
     <Card className="mt-6 w-full md:w-96" index={index}>
  <CardBody>
    {task ? (
      <img
  ...
  <CardFooter className="pt-0 flex flex-col sm:flex-row gap-2 items-center">
    <ViewTask
      title={task.title}
    ...
    />
    <EditTask taskId={task._id} title={task.title} description={task.description} />
    <Button
      color="red"
      size="sm"
      variant="outlined"
      className="flex items-center gap-2" onClick={handleDeleteCard}
    >
      delete
    </Button>
  </CardFooter>
